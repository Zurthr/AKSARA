...
export interface LiteratureBookUpdateData
  extends Partial<LiteratureBookCreateData> {}
...
export const useLiterature = () => {
  ...
  const getAllBooks = async (
    ...
    try {
      const response = await api.get<LiteratureBooksResponse>(
        `/literatures?page=${page}&per_page=${perPage}`,
      );
      return response;
    } catch (err) {
      const errorMsg =
  ...
  const getBookById = async (
    ...
    try {
      ...
      const response = await api.get<{ data: LiteratureBook }>(
        `/resources/${encodedId}`,
      );
  ...
  const createBook = async (
    bookData: LiteratureBookCreateData,
  ): Promise<LiteratureBook | null> => {
    setLoading(true);
    setError(null);

    try {
      const response = await api.post<{ data: LiteratureBook }>(
        "/books",
        bookData,
      );
      return response.data;
  ...
  const updateBook = async (
    id: number | string,
    bookData: LiteratureBookUpdateData,
  ): Promise<LiteratureBook | null> => {
    setLoading(true);
    setError(null);
    ...
    try {
      const encodedId = encodeURIComponent(String(id));
      const response = await api.put<{ data: LiteratureBook }>(
        `/books/${encodedId}`,
        bookData,
      );
      return response.data;
...
export function useLazyBooks(pageSize: number = 12) {
  ...
  const loadMore = async () => {
    ...
    try {
      const response = await $fetch<LiteratureBook[]>(
